...
export type BlitzMiddleware = (handler: NextApiHandler) => (req: NextApiRequest, res: MiddlewareResponse, ctx?: TemporaryAny) => Promise<void>;

// todo: better naming
export type Middleware = (req: NextApiRequest, res: MiddlewareResponse, next: (error?: Error) => Promise<void> | void) => Promise<void> | void
...
type GSSPHandler = (req: TemporaryAny, res: TemporaryAny, ctx: TemporaryAny) => Promise<TemporaryAny>;
...
export const setupBlitz = ({ plugins }: SetupBlitzOptions) => {
  ...
  const gSSP = (handler: GSSPHandler): GetServerSideProps => async ({ req, res }) => {
    await runMiddlewares(plugins.flatMap(p => p.middlewares), req as TemporaryAny, res as TemporaryAny)
    return handler(req, res, (res as TemporaryAny).blitzCtx);
  };

  const gSP = (handler: GSSPHandler): GetServerSideProps => async ({ req, res }) => {
    await runMiddlewares(plugins.flatMap(p => p.middlewares), req as TemporaryAny, res as TemporaryAny)
    return handler(req, res, (res as TemporaryAny).blitzCtx);
  };
