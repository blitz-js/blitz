...
export const harnessServerProps = (Controller: ControllerInstance) => (
  ctx: any,
) => {
...
export const harnessController = (Controller: ControllerInstance) => async (
  ...
  console.log('');
  console.log(
    `Started ${req.method} "${req.url}" for ${
      req.connection.remoteAddress
    } at ${new Date()}`,
  );

  const stringId =
    req.query && (Array.isArray(req.query.id) ? req.query.id[0] : req.query.id);
  delete req.query.id;
  ...
  if (result?.data) {
    if (req.url.includes('/api/')) {
      ...
      console.log(
        `  Returning to page after ${duration}: ${JSON.stringify(returning)}`,
      );
      console.log(' ');
      return returning;
    }
  ...
  const duration = prettyMs(new Date().getTime() - startTime);
  console.log(
    `Completed ${res.statusCode} ${res.statusMessage} in ${duration}`,
  );
  console.log(' ');
};
