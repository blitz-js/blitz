...
<Card type="info">
  We create a shared `validations.ts` file because we cannot import anything from a query (or
  mutation) file other than the query itself into the client. You can read more about why in [Query
